...
describe('SubscriptionMatching.scoreFilter', () => {
  it('matches an exact category and scores 20', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { categoryId: 38 }, []);
    expect(scoreFilter(filter, exp())).toBe(20);
  });

  it('returns null when category does not match', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { categoryId: 99 }, []);
  ...
  it('adds receiver weight (15) on substring ILIKE match', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { receiver: 'flix' }, []);
    expect(scoreFilter(filter, exp())).toBe(15);
  });

  it('rejects when receiver does not match', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { receiver: 'Spotify' }, []);
    expect(scoreFilter(filter, exp())).toBeNull();
  });

  it('combines category + receiver to score 35', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { categoryId: 38, receiver: 'Net' }, []);
    expect(scoreFilter(filter, exp())).toBe(35);
  });

  it('treats missing receiver as a non-match for receiver constraints', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { receiver: 'Netflix' }, []);
    expect(scoreFilter(filter, exp({ receiver: '' }))).toBeNull();
  });

  it('matches title ILIKE separately from receiver', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { title: 'flix' }, []);
    expect(scoreFilter(filter, exp())).toBe(15);
  });

  it('search matches against either title or receiver and scores 10', () => {
    const filter = buildSubscriptionFilter('report', 1, { search: 'flix' }, []);
  ...
  it('type filter rejects non-matching types', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { type: 'income' }, []);
  ...
  it('userId filter respects exact match', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { userId: 1 }, []);
  ...
  it('confirmed filter rejects opposite values', () => {
    const filter = buildSubscriptionFilter('recurring', 1, { confirmed: true }, []);
  ...
  it('matches a fully empty filter against everything (score 0)', () => {
    const filter = buildSubscriptionFilter('recurring', 1, {}, []);
...
describe('SubscriptionMatching.pickWinningFilter', () => {
  it('picks the highest-scoring filter', () => {
    const broad = buildSubscriptionFilter('report', 1, { categoryId: 38 }, []);
    const narrow = buildSubscriptionFilter(
      'recurring',
      2,
      { categoryId: 38, receiver: 'Netflix' },
      [],
    );
    expect(pickWinningFilter(exp(), [broad, narrow])).toBe(narrow);
  });

  it('on tie prefers recurring over report', () => {
    const recurring = buildSubscriptionFilter('recurring', 5, { categoryId: 38 }, []);
    const report = buildSubscriptionFilter('report', 1, { categoryId: 38 }, []);
    expect(pickWinningFilter(exp(), [report, recurring])).toBe(recurring);
  });

  it('on tie within source prefers lower id (older subscription)', () => {
    const a = buildSubscriptionFilter('recurring', 7, { categoryId: 38 }, []);
    const b = buildSubscriptionFilter('recurring', 3, { categoryId: 38 }, []);
    expect(pickWinningFilter(exp(), [a, b])).toBe(b);
  });

  it('returns null when no filter matches', () => {
    const f = buildSubscriptionFilter('recurring', 1, { categoryId: 99 }, []);
...
describe('SubscriptionMatching.assignExpensesToSubscriptions', () => {
  it('routes each expense to the most specific filter', () => {
    const broadReport = buildSubscriptionFilter(
      'report',
      1,
      { categoryId: 30, includeSubCategories: true },
      [30, 38, 39],
    ...
    const wins = assignExpensesToSubscriptions(expenses, [broadReport, narrow]);
    expect(wins.get(filterKey(narrow))?.map(e => e.id)).toEqual([1]);
    expect(wins.get(filterKey(broadReport))?.map(e => e.id)).toEqual([2]);
    expect([...wins.keys()].length).toBe(2);
  });

  it('omits expenses that match no filter', () => {
    const f = buildSubscriptionFilter('recurring', 1, { categoryId: 38 }, []);
