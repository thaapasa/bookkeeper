...
function getAll() {
    return db.queryList("SELECT id, email, firstname, lastname FROM users");
}

function getById(userId) {
    return db.queryObject("SELECT id, email, firstname, lastname FROM users WHERE id=$1", [userId])
}
